...
local error = function(text) vlib:error(text) end
local errorf = function(text, ...) vlib:errorf(text, ...) end
local log = function(text) vlib:log(text, "[camp]") end
local logf = function(text, ...) vlib:logf(text, "[camp]", ...) end
...
function pr_manager:create_uic(pr_obj, faction_key)
    ...
    local uic = core:get_or_create_component(pr_key, self.pr_path, parent)
...
function pr_manager:adjust_tooltip(pooled_resource_key, faction_key)
    ...
    if pr_interface:is_null_interface() then
        return vlib:errorf("Trying to create Pooled Resource %q for %q, but the faction doesn't have access to this PR!", pooled_resource_key, faction_key)
    end
    ...
    local function new_factor(key, state)
        local uic_path
        local parent
    ...
    for key, value in pairs(factors) do
        if value >= 0 then
            -- positive path
            new_factor(key, "positive")
        elseif value < 0 then
            -- negative path
            new_factor(key, "negative")
        end
    end
end
...
function pr_manager:init()
    ...
    if self:faction_has_resources(faction) then
        ...
        for i = 1, #resource_keys do
            ...
            self:create_uic(self:get_pr_with_key(resource_key), faction_key)
        end
    end
end
...
function TechProto:set_child_techs(children, filters)
    if not is_table(children) then
        return errorf("Trying to set children for technology w/ key [%s] but the child_techs arg provided [%s] is not a table!", self:get_key(), tostring(children))
    end
...
function TechProto:add_child_tech(child_tech)
    if not is_tech(child_tech) then
        return errorf("Trying to add child tech to tech %q, but the child tech provided [%s] isn't a valid technology!", self:get_key(), tostring(child_tech))
    end
...
function Counselor:handle_filters(filters)
    ...
    if not is_table(filters) then
        return errorf("Attempting to call Counselor:handle_filters(), but the filters table passed isn't actually a table!")
    end
    ...
    local function handle_culture(key, is_sub)
        if not is_string(key) then
            return errorf("Trying to run Counselor:handle_filters(), but the key for handle_culture isn't a string!")
        end
        ...
        if not first_faction then
            return errorf("Running Counselor:handle_filters(), but the culture key [%s] passed to handle_culture doesn't have any valid factions in the game!", key)
        end
    ...
    local function handle_faction(key)
        if not is_string(key) then
            return errorf("Running Counselor:handle_filters(), but the faction key [%s] passed isn't a valid string", tostring(key))
        end

        local test = cm:get_faction(key)
        if not test then
            return errorf("Running Counselor:handle_filters(), but the faction key [%s] passed to handle_faction doesn't have any valid faction in the game!", key)
        end
...
function tech_ui:set_tech_node_state(tech_obj, is_lock, affect_children)
    ...
    if not is_uicomponent(uic) then
        errorf("Can't find a tech UIC w/ key %q", tech_key)
        return false
    end
...
function unit_ui:get_cp()
    ...
    return errorf("Trying to get the Construction Popup within the Unit UI, but can't find any!")
end
...
function unit_ui:building_hover_listener(is_settlement_panel)
	...
	if not parent then
		return errorf("In unit_ui:building_hover_listener(), but there's no main_settlement_panel found!")
	end
	...
	core:add_listener(
		...
		function(context)
			...
			vlib:callback(
				...
				5,
				"VLIB_BuildingHovered"
			) end) if not ok then errorf(err) end
		end,
		true
	)
end
...
local function init_listeners()
    ...
    core:add_listener(
        ...
        "ComponentMouseOn",
        function(context)
            return pr_manager:get_pr_with_key(context.string)
        end,
        function(context)
            ...
            vlib:callback(function()
                ...
                local ok, err = pcall(function()
                pr_manager:adjust_tooltip(str, cm:get_local_faction_name(true))
    ...
    core:add_listener(
        ...
        function(context)
            return context:faction():name() == cm:get_local_faction_name(true) and pr_manager:get_pr_with_key(context:resource():key())
        end,
        function(context)
            pr_manager:check_value(context:resource():key(), cm:get_local_faction_name(true))
        end,
        true
    )
    ...
    core:add_listener(
        ...
        function(context)
            local faction = context:faction()
            local faction_key = faction:name()
            if pr_manager:faction_has_resources(faction) then
                local resource_keys = pr_manager:get_resource_keys_for_faction(faction)
                for i = 1, #resource_keys do
                    local resource_key = resource_keys[i]

                    if not pr_manager:get_uic(resource_key) then
                        -- create UIC, if it doesn't exist already
                        pr_manager:create_uic(pr_manager:get_pr_with_key(resource_key), faction_key)
                    else -- check value if it does
                        pr_manager:check_value(resource_key, faction_key)
                    end
                end
            end
        end,
...
function Counselor:restrict_units_for_faction(unit_table, faction_key, is_disable)
    ...
    if not is_string(faction_key) then
        return errorf("Calling restrict_units_for_faction, but the faction key provided [%s] isn't a string!", tostring(faction_key))
    end
    ...
    local function func()
        ...
        if not faction then
            return errorf("Calling restrict_units_for_faction, but the faction provided [%s] doesn't exist in this game!", tostring(faction_key))
        end
...
function Counselor:restrict_techs_for_faction(techs, faction_key, is_disable)
    ...
    if not is_string(faction_key) then
        return errorf("Calling restrict_tech_for_faction, but the faction key provided [%s] isn't a string!", tostring(faction_key))
    end
    ...
    local function func()
        local faction = cm:get_faction(faction_key)
        if not faction then
            return errorf("Calling restrict_tech_for_faction, but the faction provided [%s] doesn't exist in this game!", tostring(faction_key))
        end
...
function Counselor:add_pr_uic(pooled_resource_key, pr_icon_path, filters)
    if not is_string(pooled_resource_key) then
        return errorf("Calling add_pr_uic(), but the PR key provided [%s] isn't a valid string!", tostring(pooled_resource_key))
    end
    
    if not is_string(pr_icon_path) then
        return errorf("Calling add_pr_uic(), but the PR icon path provided [%s] isn't a valid string!", tostring(pr_icon_path))
    end
    
    local proper_filters = self:handle_filters(filters)
    if not proper_filters then
        return errorf("Calling add_pr_uic(), but the filters provided aren't valid!")
    end

    local new_pr = pr_manager:create_new_pr(pooled_resource_key, pr_icon_path)
end
...
function Counselor:set_mutually_exclusive_techs(tech_table, filters)
    ...
    if not is_table(tech_table) then
        return errorf("Calling set_mutually_exclusive_techs, but the tech table provided isn't actually a table!")
    end

    if next(tech_table) == nil then
        return errorf("Calling set_mutually_exclusive_techs, but the tech table provided is empty!")
    end

    local proper_filters = self:handle_filters(filters)
    if not is_table(proper_filters) then
        return errorf("Calling set_mutually_exclusive_techs, but the filters provided aren't valid!")
    end
...
function Counselor:set_tech_unit_unlock(tech_key, unit_table, is_unlock, filters)
    local ok, err = pcall(function()
    if not is_string(tech_key) then
        return errorf("Calling set_tech_unit_unlock(), but the tech key provided [%s] isn't a valid string!", tostring(tech_key))
    end
    ...
    if not is_boolean(is_unlock) then
        return errorf("Calling set_tech_unit_unlock(), but the is_unlock arg provided [%s] isn't a valid boolean!", tostring(is_unlock))
    end

    local proper_filters = self:handle_filters(filters)
    if not is_table(proper_filters) then
        return errorf("Calling set_tech_unit_unlock(), but the filters table provided isn't actually a table!")
    end
...
function Counselor:set_techs_lock_state(tech_table, lock_state, lock_reason, filters)
    ...
    if not is_table(tech_table) and not is_string(tech_table[1]) then
        return errorf("Calling set_techs_lock_state(), but the tech_table provided isn't a table, or doesn't have only strings (tech keys) within it.")
    end

    if not is_string(lock_state) then
        return errorf("Calling set_techs_lock_state(), but the lock_state provided [%s] isn't a valid string!", lock_state)
    end

    if not states_to_numbers[lock_state] then
        return errorf("Calling set_techs_lock_state(), but the lock_state provided [%s] isn't a valid state! Valid states are %q, %q, and %q.", lock_state, "unlocked", "locked", "disabled")
    end

    local new_state = states_to_numbers[lock_state]

    if new_state >= 2 and not is_string(lock_reason) then
        return errorf("Calling set_techs_lock_state(), but the lock_state is [%s] and requires a lock_reason, but no lock_reason was provided or the one provided [%s] isn't a string!", lock_state, tostring(lock_reason))
    end

    local proper_filters = self:handle_filters(filters)
    if not is_table(proper_filters) then
        return errorf("Calling set_techs_lock_state(), but the filters provided aren't valid!")
    end
    ...
    end end) if not ok then errorf(err) end
end
...
function Counselor:set_units_lock_state(unit_keys, lock_state, lock_reason, filters)
    ...
    if not is_table(unit_keys) and not is_string(unit_keys[1]) then
        return errorf("Calling set_units_lock_state(), but the unit_keys provided isn't a table, or doesn't have only strings (unit keys) within it.")
    end

    if not is_string(lock_state) then
        return errorf("Calling set_units_lock_state(), but the lock_state provided [%s] isn't a valid string!", lock_state)
    end

    local state = states_to_numbers[lock_state]
    if not state then
        return errorf("Calling set_units_lock_state(), but the lock_state provided [%s] isn't a valid state! Valid states are %q, %q, and %q.", lock_state, "unlocked", "locked", "disabled")
    end


    if state == 2 and not is_string(lock_reason) then
        return errorf("Calling set_units_lock_state(), but the lock_state is [%s] and requires a lock_reason, but no lock_reason was provided or the one provided [%s] isn't a string!", lock_state, tostring(lock_reason))
    end

    local proper_filters = self:handle_filters(filters)
    if not proper_filters then
        return errorf("Calling set_units_lock_state(), but the filters provided aren't valid!")
    end
...
vlib:add_manager("camp_counselor", Counselor)
