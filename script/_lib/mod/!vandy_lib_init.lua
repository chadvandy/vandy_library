...
local vandy_lib = {
    ...
    helpers_path = "script/vandy_lib/helpers",

    module_path = "script/vandy_lib/modules/",
    _modules = {},
    _prototypes = {},
...
function vandy_lib:load_modules(path)
    ...
    for filename in string.gmatch(file_str, '([^,]+)') do
        ...
        self:load_module(filename, filename_for_out)
    end
end

function vandy_lib:add_manager(name, manager)
...
function vandy_lib:load_module(module_name, full_path)
...
function vandy_lib:trigger_popup(key, text, two_buttons, button_one_callback, button_two_callback)
    ...
    if not is_string(key) then
        self:error("trigger_popup() called, but the key passed is not a string!")
        return false
    end
    ...
    if not is_string(text) then
        self:error("trigger_popup() called, but the text passed is not a string!")
        return false
    end
    ...
    if not is_boolean(two_buttons) then
        self:error("trigger_popup() called, but the two_buttons arg passed is not a boolean!")
        return false
    end
    ...
    local function do_stuff()
        ...
        core:add_listener(
            ...
            function(context)
                -- close the popup
				local ok, err = pcall(function() delete_component(popup) end) if not ok then self:error(err) end
...
function vandy_lib:init()
    ...
    self:logf("Loading all Vandy Lib modules!")
    
    local ok, err = pcall(function()
        self:load_modules(self.helpers_path)
        self:load_modules(self.module_path)
    end) if not ok then self:error(err) return end
    ...
    if __game_mode == __lib_type_frontend then
        core:add_ui_created_callback(function()
            if not _G.VANDY_LIB_FOUND then
                self:trigger_popup(
                    "no_vlib_found",
